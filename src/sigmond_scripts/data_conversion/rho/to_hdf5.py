...
def main():
  for ensemble in defs.ensembles:
    ...
    all_equal = all(chs == channels[0] for chs in channels)
    if not all_equal:
      print("not all equal\n\n")
      exit()
    ...
    for channel in tqdm.tqdm(channels):
      ...
      for replica in ensemble.replica:
        ...
        for source in ensemble.sources:
          ...
          hdf5_file = get_hdf5_file(replica_ensemble_name, source)
...
def get_hdf5_file(ensemble_name, source):
  output_dir = os.path.join(defs.output_dir, ensemble_name)
  os.makedirs(output_dir, exist_ok=True)

  hdf5_file = os.path.join(output_dir, f"{ensemble_name}_t0{source[0]}_{defs.parity_name[source[1]]}.hdf5")
  return hdf5_file
