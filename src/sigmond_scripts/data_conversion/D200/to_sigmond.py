    ...
    for corr_num, correlator in enumerate(tqdm.tqdm(correlators), 1):
      ...
      for replica in ensemble.replica:
        ...
        for source in ensemble.sources:
          ...
          correlator_data = get_data(correlator, data_files, replica_ensemble_name, replica, ensemble.Nt, source[0], source[1], args.ensembles_file)
      ...
      binfile = get_binfile(correlator, ensemble.name, args.output)
      ...
      if corr_num % MAX_CORRS == 0:
        write_multiple_data(data_to_write, ensemble.name)
...
def get_data(correlator, data_files, ensemble_name, replica, ensemble_Nt, tsrc, forward, ensembles_file):
  if not forward:
