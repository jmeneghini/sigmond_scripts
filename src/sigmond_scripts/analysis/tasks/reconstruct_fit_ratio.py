...
class ReconstructFitRatio(tasks.task.Task):
  """ The ReconstructFitRatio Task!
  """
  task_type = "reconstruct_fit_ratio"
  ...
  def readConfig(self, **task_options):
    """readConfig function for ReconstructFitRatio
  ...
  def getSigmondInputs(self):
    ...
    for operator_set, fit_infos in self.spectrum.items():
      ...
      for level, fit_info in enumerate(fit_infos):
        ...
        if fit_info.non_interacting_operators is not None: #must have 2
        ...
        this_rat_op = fit_info.operator.ratio_op
        this_fitrat_op = fit_info.operator.fit_ratio_op
        sigmond_input.doReconstructRatioFromMultiExpFit( this_rat_op, this_fitrat_op,
            fit_info, scattering_particle_fit_info[0], scattering_particle_fit_info[1], 
...
#     self._setEnergies()
    doc = util.create_doc(f"FitRatio: {self.ensemble_name} - {self.task_name}", True)
#     if self.spectrum:
...
#       self._addZfactors(doc)
    ...
    filename = os.path.join(self.results_dir, f"ratios_{self.ensemble_name}_{self.task_name}_rebin{self.rebin}")
...
#                     util.add_image(fig, self.results_dir, plot_file, width='1.0', caption=cap)
      ...
      for operator_set, fit_infos in self.spectrum.items():
        ...
        with doc.create(pylatex.Subsection(str(operator_set))):
          levels = list(range(len(fit_infos)))
